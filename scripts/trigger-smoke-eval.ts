...
export async function triggerSmokeEval(argv = process.argv.slice(2), env = process.env): Promise<void> {
  ...
  await client.send(new PutEventsCommand({ Entries: [{ EventBusName: env.EVENT_BUS_NAME ?? 'healthcare-rag-smoke-events', Source: 'healthcare-rag.manual', DetailType: 'eval.run.requested', Detail: JSON.stringify({ maxCases, budgetUsd }) }] }));
