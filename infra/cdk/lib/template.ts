...
type Parameter = { Type: string; Description?: string; NoEcho?: boolean; MinLength?: number };
export type Template = { AWSTemplateFormatVersion: string; Description: string; Parameters: Record<string, Parameter>; Resources: Record<string, Resource>; Outputs: Record<string, unknown> };
...
export function synthTemplate(profile: InfraProfile, flags: InfraFlags): Template {
  ...
  const resources: Record<string, Resource> = {
    ...
    EvalRequestedRule: { Type: 'AWS::Events::Rule', Properties: { EventBusName: { Ref: 'EventBus' }, EventPattern: { source: ['healthcare-rag.manual'], 'detail-type': ['eval.run.requested'] }, State: 'ENABLED', Targets: [{ Id: 'EvalRunner', Arn: { 'Fn::GetAtt': ['EvalFunction', 'Arn'] } }] } },
    ...
    AuthorizerLambdaRole: { Type: 'AWS::IAM::Role', Properties: { AssumeRolePolicyDocument: { Statement: [{ Effect: 'Allow', Principal: { Service: 'lambda.amazonaws.com' }, Action: 'sts:AssumeRole' }] }, PermissionsBoundary: flags.permissionsBoundaryArn, Policies: [{ PolicyName: 'authorizer-log-write', PolicyDocument: { Statement: [{ Effect: 'Allow', Action: ['logs:CreateLogStream', 'logs:PutLogEvents'], Resource: '*' }] } }] } },
    ChatLogGroup: { Type: 'AWS::Logs::LogGroup', Properties: { LogGroupName: '/aws/lambda/healthcare-rag-chat-smoke', RetentionInDays: 7 } },
    AuthorizerLogGroup: { Type: 'AWS::Logs::LogGroup', Properties: { LogGroupName: '/aws/lambda/healthcare-rag-smoke-authorizer', RetentionInDays: 7 } },
    EvalLogGroup: { Type: 'AWS::Logs::LogGroup', Properties: { LogGroupName: '/aws/lambda/healthcare-rag-eval-smoke', RetentionInDays: 7 } },
    SmokeAuthorizerFunction: { Type: 'AWS::Lambda::Function', DependsOn: 'AuthorizerLogGroup', Properties: { Runtime: 'nodejs22.x', Handler: 'index.handler', Role: { 'Fn::GetAtt': ['AuthorizerLambdaRole', 'Arn'] }, Timeout: 5, MemorySize: 128, Environment: { Variables: { SMOKE_API_KEY: { Ref: 'SmokeApiKey' } } }, Code: { ZipFile: "exports.handler = async (event) => ({ isAuthorized: (event.headers?.['x-smoke-api-key'] || event.headers?.['X-Smoke-Api-Key']) === process.env.SMOKE_API_KEY });" } } },
    ChatFunction: { Type: 'AWS::Lambda::Function', DependsOn: 'ChatLogGroup', Properties: { Runtime: 'nodejs22.x', Handler: 'index.handler', Role: { 'Fn::GetAtt': ['ChatLambdaRole', 'Arn'] }, Timeout: 20, MemorySize: 1024, VpcConfig: { SecurityGroupIds: [{ Ref: 'LambdaSecurityGroup' }], SubnetIds: [{ Ref: 'IsolatedSubnetA' }] }, Environment: { Variables: { PROFILE: profile, AWS_SMOKE_USE_REAL_ADAPTERS: 'true', AUDIT_TABLE: { Ref: 'AuditTable' }, INDEX_BUCKET: { Ref: 'CorpusBucket' }, INDEX_KEY: 'index/index-artifact.json.gz', SMOKE_API_KEY: { Ref: 'SmokeApiKey' } } }, Code: { S3Bucket: { Ref: 'LambdaArtifactBucket' }, S3Key: { Ref: 'ChatLambdaArtifactKey' } } } },
    EvalFunction: { Type: 'AWS::Lambda::Function', DependsOn: 'EvalLogGroup', Properties: { Runtime: 'nodejs22.x', Handler: 'index.handler', Role: { 'Fn::GetAtt': ['ChatLambdaRole', 'Arn'] }, Timeout: 120, MemorySize: 1024, VpcConfig: { SecurityGroupIds: [{ Ref: 'LambdaSecurityGroup' }], SubnetIds: [{ Ref: 'IsolatedSubnetA' }] }, Environment: { Variables: { PROFILE: profile, AWS_SMOKE_USE_REAL_ADAPTERS: 'true', AUDIT_TABLE: { Ref: 'AuditTable' }, INDEX_BUCKET: { Ref: 'CorpusBucket' }, INDEX_KEY: 'index/index-artifact.json.gz', EVAL_OUT: '/tmp/aws-smoke-eval.json' } }, Code: { S3Bucket: { Ref: 'LambdaArtifactBucket' }, S3Key: { Ref: 'EvalLambdaArtifactKey' } } } },
    HttpApi: { Type: 'AWS::ApiGatewayV2::Api', Properties: { Name: `healthcare-rag-${profile}`, ProtocolType: 'HTTP', CorsConfiguration: { AllowOrigins: ['http://localhost:3000', 'http://127.0.0.1:8787'], AllowMethods: ['POST'], AllowHeaders: ['content-type', 'x-smoke-api-key'] } } },
    ...
    SmokeApiAuthorizer: { Type: 'AWS::ApiGatewayV2::Authorizer', Properties: { ApiId: { Ref: 'HttpApi' }, AuthorizerType: 'REQUEST', AuthorizerPayloadFormatVersion: '2.0', EnableSimpleResponses: true, IdentitySource: ['$request.header.x-smoke-api-key'], Name: 'smoke-api-key-authorizer', AuthorizerUri: { 'Fn::Sub': 'arn:aws:apigateway:${AWS::Region}:lambda:path/2015-03-31/functions/${SmokeAuthorizerFunction.Arn}/invocations' } } },
    ChatRoute: { Type: 'AWS::ApiGatewayV2::Route', Properties: { ApiId: { Ref: 'HttpApi' }, RouteKey: 'POST /chat', AuthorizationType: 'CUSTOM', AuthorizerId: { Ref: 'SmokeApiAuthorizer' }, Target: { 'Fn::Sub': 'integrations/${ChatIntegration}' } } },
    ApiStage: { Type: 'AWS::ApiGatewayV2::Stage', Properties: { ApiId: { Ref: 'HttpApi' }, StageName: '$default', AutoDeploy: true, DefaultRouteSettings: { ThrottlingBurstLimit: 2, ThrottlingRateLimit: 1 } } },
    ApiInvokePermission: { Type: 'AWS::Lambda::Permission', Properties: { Action: 'lambda:InvokeFunction', FunctionName: { Ref: 'ChatFunction' }, Principal: 'apigateway.amazonaws.com', SourceArn: { 'Fn::Sub': 'arn:aws:execute-api:${AWS::Region}:${AWS::AccountId}:${HttpApi}/*/POST/chat' } } },
    AuthorizerInvokePermission: { Type: 'AWS::Lambda::Permission', Properties: { Action: 'lambda:InvokeFunction', FunctionName: { Ref: 'SmokeAuthorizerFunction' }, Principal: 'apigateway.amazonaws.com', SourceArn: { 'Fn::Sub': 'arn:aws:execute-api:${AWS::Region}:${AWS::AccountId}:${HttpApi}/authorizers/${SmokeApiAuthorizer}' } } },
...
function outputs(profile: string): Record<string, unknown> {
  return {
    ...
    EventBusName: { Value: { Ref: 'EventBus' } },
